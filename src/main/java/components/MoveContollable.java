...
@Override
    ...
    public void moveCommand(Transform target){
        ...
        if(mecircle!=null){
            ...
            merad=gameObject.transform.scale.x / 2+gameObject.transform.scale.y / 2;
        }
        if(circle!=null){
            closeenuf= circle.getRadius()+merad;
        }
        else {
            closeenuf = target.scale.x / 2 + target.scale.y / 2+merad;
        }
        this.movePos=target.position;
        SharedMoveCommand();
        Bigtimer= (int) (Maf.distance(movePos,super.gameObject.transform.position)/speed/ Window.physicsStep*1.5/Math.pow(acceleration,2))*30;
    }
    public void SharedMoveCommand(){
        ...
        if(acceleration==1) {
            ...
            cannaygetclosertimeout = Bigtimer;
        }
